...
genai.configure(api_key="Gimini API")
...
def draw(info,prev_pos,canvas):
    ...
    if fingers == [0,1,0,0,0]:
        ...
        cv2.line(canvas, current_pos,prev_pos,(51, 153, 255),20)
...
def sendToAI(model,canvas,fingures):
    if fingures ==[1,1,1,1,0]:
        pil_image = Image.fromarray(canvas)
        response = model.generate_content(["Guss the Number: ", pil_image])
